...
- **The MCP server closes the graph on shutdown** (`kgrag_priv` sweep item 5),
  ...
  `create_server()` is handed the `MetaKG`, so the hook closes that instance
  directly rather than looking up a module global as the other fleet servers
  do. Verified through `mcp.shared.memory`'s in-process transport: a real
...
- **The `kg` Poetry group is gone** (`kgrag_priv` sweep item 50, phase 1).
  ...
  The hook `metabokg install-hooks` writes now resolves `pycodekg` and
  `dockg-snapshot` from `PATH`, falling back to a `.venv` copy if one is
  present, and skipping cleanly when neither is -- which is what its own
  comment always said it did. `.venv/bin/metabokg` is untouched: this
  repo's own binary is a dependency, not a tool. Re-run
  `metabokg install-hooks --force` to pick it up.
...
- **`ruff` floor raised from `>=0.4.0` to `>=0.15`** inside the existing
  `<0.16` cap, and **`pre-commit` from `>=4.0.0` to `>=4.5.1`**, the fleet
  floor set by `KG_utils` (`kgrag_priv` sweep item 49, tier 1). Metabo_kg was
  the last repo below the pre-commit floor. `mcp` moves to 1.30.0 in the lock.
- **`mcp` floor raised from `>=1.0.0` to `>=1.3.0`**, keeping the `<2` cap
  (`kgrag_priv` sweep item 47). `metabokg-mcp` passes `instructions=` to
...
- **`docs/WORKFLOW.md` enrichment phases renumbered** — The Stage 3 diagram and reference tables now reflect the simplified `enrich()` pipeline: glycans and KO enzymes shift to Phase 2c/2d, and `kegg_reaction_detail.tsv` is documented as a **Standalone** fallback (`enrich_reactions_from_detail()`) that is no longer called by `enrich()`.
