...
class MetaStore:
    ...
    def upsert_kinetic_param(self, param: KineticParam) -> None:
        """
        Insert or replace a :class:`~metabokg.primitives.KineticParam` row.
    ...
    def upsert_kinetic_params(self, params: list[KineticParam]) -> int:
        """
        Bulk insert/replace a list of :class:`~metabokg.primitives.KineticParam` rows.
    ...
    def upsert_regulatory_interaction(self, ri: RegulatoryInteraction) -> None:
        """
        Insert or replace a :class:`~metabokg.primitives.RegulatoryInteraction` row.
...
class GraphStore(MetaStore):
    ...
    def query_text(self, query: str, k: int = 10) -> list[dict]:
        ...
        (vector) search use :class:`~metabokg.index.MetaIndex`.
