...
from metabokg import __version__
from metabokg.analyze import PathwayAnalysisReport
...
def render_thorough_report(report: PathwayAnalysisReport, *, markdown: bool = True) -> str:
    ...
    lines.append(
        "- **Steady-state analysis**: `metabokg-simulate fba --pathway <id>` — "
        "Predict metabolic fluxes at equilibrium\n"
    )
    lines.append(
        "- **Kinetic dynamics**: `metabokg-simulate ode --pathway <id>` — "
        "Model time-course concentration changes with enzyme kinetics\n"
    )
    lines.append(
        "- **Perturbation analysis**: `metabokg-simulate whatif --pathway <id>` — "
