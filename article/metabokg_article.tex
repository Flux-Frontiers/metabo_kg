...
% Metadata
% ---------------------------------------------------------------------------
\title{\textbf{MetaboKG: A Unified Knowledge Graph System\\for Metabolic Pathway Analysis}}
...
\begin{abstract}
\noindent
We present \textsc{MetaboKG}, a local-first metabolic knowledge graph whose
...
Beyond querying, \textsc{MetaboKG} integrates three simulation modalities
...
typed tools, making \textsc{MetaboKG} a first-class data source for AI reasoning
...
(17,058 nodes, 41,334 edges, 9,427 enzymes, 7,631 indexed embeddings)---with
...
\textsc{MetaboKG} resolves this tension with a dual-layer local knowledge graph
...
\textsc{MetaboKG} treats the knowledge graph as a reproducible,
...
\textit{Homo sapiens} metabolome (369 KEGG pathways, 17,058 nodes, 41,334
edges, 9,427 enzymes) builds in 30--60~seconds, and queries complete in
milliseconds to seconds.

\subsection{Design Goals}

\textsc{MetaboKG} is built around three core principles, in priority order:
...
\textsc{MetaboKG} represents metabolism as a directed property graph
...
\begin{table}[H]
  \centering
  \caption{Edge relation types in the MetaboKG graph schema.}
  \label{tab:relations}
  \small
  \begin{tabular}{@{}lll@{}}
    \toprule
...
data integration. \textsc{MetaboKG} assigns each node a stable, URI-style
...
(naturally vector problems) to LanceDB, \textsc{MetaboKG} avoids forcing a
...
needed, avoiding the 24~MB embedding model download unless semantic search is
actually used.
...
Pathway files (KGML / SBML / BioPAX / CSV)
  ...
  \end{lstlisting}
  \caption{MetaboKG data pipeline. Format-specific parsers produce a stream of
...
\textsc{MetaboKG} ingests metabolic pathway data from four standard formats:
...
\textsc{MetaboKG} provides an optional post-build enrichment pipeline that
...
\subsection{Multi-Phase Enrichment Pipeline}

Enrichment proceeds in three numbered phases (with Phase~2 split into four
sub-phases). All phases are idempotent and safe to re-run.

\begin{description}[leftmargin=*,itemsep=3pt]
  \item[Phase~1 (no network, always runs).]
    For every reaction node still carrying a bare accession name, the enricher
    looks up all \texttt{CATALYZES} edges pointing to it and joins the
    catalysing enzyme gene symbols with a slash separator:
    \begin{center}
      \texttt{R00710} $\to$ \texttt{ADH1A / ADH1B / ADH1C}
    \end{center}
    This phase requires no external data and runs immediately after build.
    It provides a useful human-readable label for any reaction even if the
    canonical KEGG name is unavailable.

  \item[Phase~2a --- compound names.]
    KEGG publishes a bulk compound name list ($\sim$19,500 entries).
    A provided download script fetches it once; subsequent runs read it
    locally.  This sub-phase updates all compound nodes:
    \begin{center}
      \texttt{C00031} $\to$ \texttt{D-Glucose}
    \end{center}

  \item[Phase~2b --- reaction names.]
    The KEGG bulk reaction name list ($\sim$12,400 entries) provides
    canonical IUPAC-style reaction names.  This sub-phase overrides
    Phase~1 gene-symbol labels wherever a canonical name is available:
    \begin{center}
      \texttt{R00710} $\to$ \texttt{Acetaldehyde:NAD\textsuperscript{+}
      oxidoreductase}
    \end{center}

  \item[Phase~2c --- glycan compound names.]
    KEGG glycans use a separate \texttt{gl:G\#\#\#\#\#} namespace.
    This sub-phase loads \texttt{kegg\_glycan\_names.tsv} and resolves
    \texttt{cpd:kegg:gl:G\#\#\#\#\#} nodes to human-readable names:
    \begin{center}
      \texttt{G13086} $\to$ \texttt{Lactosylceramide}
    \end{center}

  \item[Phase~2d --- KEGG Orthology enzyme names.]
    KO stub enzyme nodes (\texttt{enz:kegg:K\#\#\#\#\#}) are resolved
    using the KEGG KO description list.  The gene symbol(s) preceding the
    first semicolon are used as the enzyme label:
    \begin{center}
      \texttt{K00001} $\to$ \texttt{adh}
    \end{center}

  \item[Phase~3 --- organism gene symbols (called separately).]
    Enzyme nodes carrying bare integer Entrez gene IDs
    (e.g.\ \texttt{enz:kegg:cge:100689064}) are resolved to gene symbols
    using per-organism KEGG gene list TSVs
    (\texttt{data/\{org\}\_gene\_names.tsv}).  Organisms are detected
    automatically from enzyme node IDs; the phase also handles
    \texttt{enz:syn:\{hash\}} nodes produced by the SBML FBC parser
    (e.g.\ iCHO2441 gene products) by scanning all available
    \texttt{*\_gene\_names.tsv} files:
    \begin{center}
      \texttt{100689064} $\to$ \texttt{Ldha}
    \end{center}
    Phase~3 is invoked separately via \texttt{enrich\_enzyme\_names()} and
    is what enables \texttt{--knockout Ldha} and
    \texttt{resolve\_id("Ldha")} at query time.
    It is not called by the main \texttt{enrich()} entry point.
\end{description}

A standalone detail-fallback function (\texttt{enrich\_reactions\_from\_detail})
is also available for reactions that remain as bare accessions after
Phases~1 and~2b; it reads \texttt{kegg\_reaction\_detail.tsv} and is
invoked directly when needed.

All phases can be triggered together with the build step via the
\texttt{--enrich} flag (Phases~1--2d); Phase~3 runs automatically as part
of \texttt{metabokg-build} when gene TSV files are present:

\begin{lstlisting}[style=bash]
python scripts/download_kegg_names.py   # fetch TSV files once
metabokg-build --data ./pathways        # build + enrich (default)
\end{lstlisting}
...
The dual-layer storage architecture is what gives MetaboKG its query
...
user does not know an exact identifier. \textsc{MetaboKG} therefore maintains a
vector index over node descriptions using LanceDB~\citep{lancedb2023} as the
approximate nearest-neighbour engine and the \texttt{BAAI/bge-small-en-v1.5}
...
24~MB of disk space on first download.
...
\textsc{MetaboKG} exposes four query modalities through a unified API. These
...
Building on the structural query layer, \textsc{MetaboKG} provides three
...
The \texttt{metabokg-viz} command launches a Streamlit~\citep{streamlit2019}
...
The \texttt{metabokg-viz3d} command launches a PyVista~\citep{sullivan2019pyvista}
...
functions. \textsc{MetaboKG} implements an MCP server exposing the knowledge
...
We demonstrate \textsc{MetaboKG} on the complete \textit{Homo sapiens}
...
nodes       : 17058  {'compound': 5115, 'enzyme': 9427,
              'pathway': 369, 'reaction': 2147}
edges       : 41334  {'CATALYZES': 2579, 'CONTAINS': 33255,
              'PRODUCT_OF': 2745, 'SUBSTRATE_OF': 2755}
indexed     : 7631 vectors  dim=384
\end{lstlisting}

By default, \texttt{metabokg-build} wipes the existing database and vector index
...
command \texttt{metabokg-update --data ./pathways}.
...
Output (with default name enrichment applied during build; the KGML parser
now splits multi-R \texttt{<reaction>} elements into separate reaction nodes,
so each row corresponds to a single canonical KEGG R-number with its
canonical name):
\begin{lstlisting}[style=bash, caption={}]
Pyruvate
  pyruvate:thiamin diphosphate acetaldehyd  SUBSTRATE_OF
  pyruvate:carbon-dioxide ligase (ADP-form  SUBSTRATE_OF
  pyruvate:NAD+ 2-oxidoreductase (CoA-acet  SUBSTRATE_OF
  acetyl-CoA:formate C-acetyltransferase    SUBSTRATE_OF
  pyruvate:ferredoxin 2-oxidoreductase (Co  SUBSTRATE_OF
  ... and 38 more
Substrates: ['Phosphoenolpyruvate']
Products:   ['Pyruvate']
\end{lstlisting}

Cofactors (ADP, ATP) are not present in the stoichiometry block returned for
\texttt{R00200} in this KGML graph; the canonical KEGG entry encodes the
phosphoryl transfer with substrate-product mapping limited to the carbon
backbone. They are recoverable from the SBML or BioPAX import paths when
...
Output (with default name enrichment applied during build; the bidirectional
BFS finds an 8-hop route through the alpha/beta-D-glucose-6-phosphate
isomerisation and the pentose-phosphate / TCA crossover via NADPH and
2-oxoglutarate, which is shorter than the canonical glycolysis route under
the current edge set):
\begin{lstlisting}[style=bash, caption={}]
Path length: 8 steps
  compound   D-Glucose
  reaction   ATP:D-glucose 6-phosphotransferase
  compound   D-Glucose 6-phosphate
  reaction   D-glucose-6-phosphate aldose-ketose-isomerase
  compound   D-Fructose 6-phosphate
  reaction   alpha-D-glucose-6-phosphate aldose-ketose-isomerase
  compound   alpha-D-Glucose 6-phosphate
  reaction   alpha-D-glucose 6-phosphate ketol-isomerase
  compound   beta-D-Glucose 6-phosphate
  reaction   beta-D-glucose-6-phosphate:NADP+ 1-oxoreductase
  compound   NADPH
  reaction   glutathione:NADP+ oxidoreductase
  compound   NADP+
  reaction   isocitrate:NADP+ oxidoreductase (decarboxylating)
  compound   2-Oxoglutarate
  reaction   L-alanine:2-oxoglutarate aminotransferase
  compound   Pyruvate
\end{lstlisting}

The query resolves in milliseconds on the local SQLite index. The algorithm
scales efficiently to the complete human metabolome (17,058 nodes, 41,334
...
Output (\texttt{BAAI/bge-small-en-v1.5} encoder; cosine distance):
\begin{lstlisting}[style=bash, caption={}]
Fatty acid degradation                    dist=0.567
\end{lstlisting}
...
\texttt{query\_pathway} filters \texttt{index.search} hits to
\texttt{kind=="pathway"}, so when non-pathway nodes (compounds with overlapping
terms) rank highly the result count may be smaller than \texttt{k}.

\subsection{Pathway Analysis Report}

The \texttt{metabokg-analyze} command runs a seven-phase analysis and produces a
structured Markdown report:

\begin{lstlisting}[style=bash]
$ metabokg-analyze --output analysis.md --top 10
\end{lstlisting}
...
\begin{lstlisting}[style=bash]
git clone https://github.com/Flux-Frontiers/metabo_kg
cd metabo_kg
...
straightforward. Running \texttt{metabokg-build} with \texttt{--no-wipe} (or using
the convenience command \texttt{metabokg-update}) issues \texttt{INSERT OR REPLACE}
statements, updating existing nodes and appending new ones without duplicating entries.

\textbf{Codebase self-analysis.} \textsc{MetaboKG} is itself analysed with its
...
enables navigating the MetaboKG implementation via natural-language queries and
confirms that the two tools share compatible architectural patterns. On the
\textsc{MetaboKG} codebase, the \textsc{CodeKG} static analysis produces 3,136
...
Several deliberate design trade-offs distinguish \textsc{MetaboKG} from existing
...
\textsc{MetaboKG} delivers both capabilities in a self-contained package suited
...
\textsc{MetaboKG} produces self-contained graphs. The same input files always
...
\textbf{Snapshot-based operation.} \textsc{MetaboKG} is a local analysis tool,
not a live database mirror. The knowledge graph is a snapshot at build time;
users must re-run \texttt{metabokg-build} after updating source files. This
...
\textbf{Stoichiometric models and simulations.} \textsc{MetaboKG} provides
...
\textit{Homo sapiens} metabolome (369 KEGG pathways, 17,058 nodes, 41,334
edges, 7,631 vector embeddings) shows the following performance
characteristics:

\begin{table}[H]
  \centering
  \caption{Performance on the complete human metabolome (17K nodes, 41K edges).
  ...
  \begin{tabular}{@{}lll@{}}
    ...
    Semantic search (natural language) & 100--500ms & Vector similarity on 7.6K nodes \\
    Shortest-path (6 hops) & 10--50ms & BFS on 41K edges \\
...
\textsc{MetaboKG} fills a gap in metabolic data integration. Existing systems
...
provided). \textsc{MetaboKG} breaks this pattern with a dual-layer local
...
makes \textsc{MetaboKG} especially well suited to research workflows where
...
We expect \textsc{MetaboKG} to be immediately useful as: (1)~a foundation for
...
\url{https://github.com/Flux-Frontiers/metabo_kg} under the Elastic License~2.0.
...
For custom or unpublished data, \textsc{MetaboKG} accepts flat tables with a
