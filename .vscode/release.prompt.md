---
mode: 'agent'
description: 'Create a versioned release: promote CHANGELOG, bump version, write release notes, commit, tag, and push.'
---

# Release Workflow

Create a new versioned release by promoting the `[Unreleased]` section of `CHANGELOG.md` into a dated version entry, writing `release-notes.md`, committing the changes, tagging the commit, and pushing the tag to the remote. Execute the following steps in sequence.
...
In `README.md`, find the version badge line and replace `<current_version>` with `<new_version>`.
...
3. Open `docs/analysis_v<new_version>.md` and ensure the header contains:
   ...
   **Generated:** <today's date in YYYY-MM-DD>
   ```
4. Delete any previous `docs/analysis_v<old_version>.md` if it exists.
