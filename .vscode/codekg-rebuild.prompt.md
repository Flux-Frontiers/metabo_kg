---
mode: 'agent'
description: 'Wipe and rebuild the CodeKG SQLite knowledge graph and LanceDB semantic index for a repository.'
---
...
**Usage:**
- No argument — Rebuild for the current working directory
- With path argument — Rebuild for the specified repository
...
1. `codekg-build-sqlite` (preferred if inside an activated venv)
2. `poetry run codekg-build-sqlite` (fallback)
